...
function checkConflictReiModuleVersion() {
    for (let [reiModule, versions] of Object.entries(reiModuleVersions)) {
        if (versions.length > 1) {
            console.log('detect confilict module versions, module: ', reiModule, ', versions: ', versions);
            return false;
        }
    }
    return true
...
let updatedReiModules = new Set(['@rei-network/core', '@rei-network/common'])
...
modulesNeedRelease.forEach((moduleNeedRelease) => {
    ...
    versionsToUpgradeReview.push([reiModule.name, reiModule.version, 'TODO: input manual'])
})
